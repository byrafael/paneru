...
| `display_fallthrough` | Boolean | `true` | When enabled, `window_focus_north`/`_south` and `window_swap_north`/`_south` fall through to the display above/below if no window exists in that direction within the current workspace. Set to `false` to treat each display as a self-contained desktop (like macOS spaces): directional commands then stop at the display edge, and windows only change display via the explicit `window_nextdisplay` / `window_nextdisplaysend` bindings. |
